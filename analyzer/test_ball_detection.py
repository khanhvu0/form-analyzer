...
def main():
    parser = argparse.ArgumentParser(
        description="Test combined pose and tennis ball detection on a video"
    )
    parser.add_argument("--input", type=str, required=True, help="Path to input video")
    parser.add_argument(
        "--output",
        type=str,
        default="output_combined_detection.mp4",
    ...
    if not args.skip_pose:
        ...
        result_generator = inferencer(
            args.input, show=False, vis_out_dir=temp_dir, radius=4, thickness=2
        )
    ...
    print(f"Running ball and racket detection on: {args.input}")
    print(f"Tennis ball is class 32, Racket is class 38 in COCO dataset")
    print(f"Using confidence threshold: {args.conf}")

    # Run inference with tracking
    ball_results = yolo_model.track(
        source=args.input,
        classes=32,  # Detect both tennis ball and racket
        conf=args.conf,
    ...
    for ball_result in ball_results:
        ...
        if ball_result.boxes is not None and len(ball_result.boxes) > 0:
            ...
            for i, box in enumerate(boxes):
                ...
                class_name = "Ball" if cls == 32 else "Racket"
                label = (
        ...
        if frame_idx % 10 == 0:
            print(
                f"Processed {frame_idx} frames, found {total_ball_detections} ball detections so far"
            )
    ...
    print(
        f"Frames with ball detections: {ball_frames} ({ball_frames/frame_idx*100:.1f}%)"
    )
